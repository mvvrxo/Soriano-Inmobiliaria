  ...
  return <main>
    <section className="home-hero-clean">
      <div className="site-shell hero-clean-grid">
        <div><p className="eyebrow">{editableCopy.hero.eyebrow}</p><h1>{editableCopy.hero.title}</h1><p>{editableCopy.hero.description}</p></div>
      </div>
      <div className="site-shell hero-search"><SearchBar /></div>
    </section>
    ...
    <section className="buy-home site-shell"><div><p className="eyebrow">{editableCopy.buy.label}</p><h2>{editableCopy.buy.title}</h2><p>{editableCopy.buy.description}</p><Link className="button-black" href="/inmuebles">Buscar vivienda <span>→</span></Link></div><div className="buy-home-side"><p>CUÉNTANOS QUÉ BUSCAS</p><span>La búsqueda está preparada para mostrar las viviendas cuando estén disponibles.</span></div></section>
